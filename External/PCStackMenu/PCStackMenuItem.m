...
- (id)initWithFrame:(CGRect)frame withTitle:(NSString *)title withImage:(UIImage *)image alignment:(UITextAlignment)alignment
{
    ...
    if (self)
	{
		CGRect rect = CGRectMake(alignment == UITextAlignmentLeft ? 0 : frame.size.width - frame.size.height, 3, frame.size.height, frame.size.height);
		...
		rect = CGRectMake(alignment == UITextAlignmentRight ? 0 : frame.size.height + 5, 3, frame.size.width - frame.size.height - 5, frame.size.height - 4);
		_stackTitleLabel = [[UILabel alloc] initWithFrame:rect];
		_stackTitleLabel.textAlignment = UITextAlignmentCenter; //alignment;
		_stackTitleLabel.font = [UIFont boldSystemFontOfSize:20];
...
//		_stackTitleLabel.shadowOffset = CGSizeMake(2, 2);
		...
		rect.size.width = labelSize.width + 30;
        rect.size.height = labelSize.height + 6;
		_stackTitleLabel.frame = rect;
		[self addSubview:_stackTitleLabel];
        
        _stackTitleLabel.layer.cornerRadius = 17;
        _stackTitleLabel.layer.masksToBounds = YES;
        _stackTitleLabel.layer.borderWidth = 1;
        _stackTitleLabel.layer.borderColor = [[UIColor darkGrayColor] CGColor];
        _stackTitleLabel.layer.backgroundColor = [[UIColor grayColor] CGColor];
