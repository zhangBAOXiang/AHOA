...
- (void)show:(PCStackMenuBlock)block
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
		for(int i = 0; i < [_items count]; i++)
		{
			...
			r.origin.y += (i+1) * r.size.height * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionCounterClockWiseUp ? -1 : 1);
//            r = [self convertRect:r toView:_parent];
			stackItem.frame = r;
...
//	self = [self initWithFrame:[UIScreen mainScreen].bounds];
    ...
    self = [self initWithFrame:CGRectMake(startPoint.x, startPoint.y, 120, itemHeight*max) inView:parent] ;
	...
	for(int i = 0; i < max; i++)
	{
		CGRect r = CGRectMake((direction == PCStackMenuDirectionClockWiseUp || direction == PCStackMenuDirectionCounterClockWiseDown) ? 0 : startPoint.x,
							  ...
							  itemHeight);
//		r = [self convertRect:r fromView:_parent];
...
{
	_overlayWindow = [[UIRotateWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
//	_overlayWindow = [[UIRotateWindow alloc] initWithFrame:CGRectMake(0, 0, 1024, 1024)];
...
-(void)createOverlayView: (UIView*)parent
{
    CGRect frame = CGRectMake(0, 0, parent.frame.size.height, parent.frame.size.width);
    [self setFrame:frame];
    _overlayView = [[UIView alloc]initWithFrame: frame];
    self.backgroundColor = [UIColor clearColor];
    [_overlayView addSubview:self];
    [parent addSubview:_overlayView];
}
...
- (id)initWithFrame:(CGRect)frame inView: (UIView*)parent
{
