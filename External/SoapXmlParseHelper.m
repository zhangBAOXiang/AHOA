...
+ (NSString*)SoapMessageResultXml:(id)data ServiceMethodName:(NSString*)methodName{
    ...
    NSString *searchStr=[NSString stringWithFormat:@"%@Response",methodName];//Response  Result Response 
    NSString *MsgResult=@"";
...
//        NSString *nodeName=node.name;
        if ([node.children count]>0)
        {
            ...
            [arr addObject:[self dictNodeChilds:node]];
            //   NSLog(@"%@",[self nodeChilds:node]);
        }else
        {
            //[arr addObject:[self nodeChilds:node]];
            [arr addObject:@""];
...
+ (NSString*)nodeChilds:(GDataXMLNode*)node
{
    NSString *arr;//=[NSMutableArray array];
    NSArray *childs=[node children];
    NSMutableDictionary *dic=[NSMutableDictionary dictionary];
    NSString *nodeName;
    for (GDataXMLNode* child in childs)
    {
        nodeName=child.name;//获取节点名称
        ...
        //[arr addObject:[child stringValue]];
        arr=[child stringValue];
    }
    
    return arr;
}

+ (NSMutableDictionary*)dictNodeChilds:(GDataXMLNode*)node
{
    NSArray *childs=[node children];
    NSMutableDictionary *dic=[NSMutableDictionary dictionary];
    NSString *nodeName;
