...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        UITableViewCell *cell1=[tableView dequeueReusableCellWithIdentifier:@"webcell"];
        
        if (indexPath.row == 0) {
            
            cell1.imageView.image=[UIImage imageNamed:@"first_page_tzck_icon.png"];
            cell1.textLabel.font=[UIFont systemFontOfSize:15];
            
            if (self.menu) {
                cell1.textLabel.text=self.menu.menuName;
            }else{
                cell1.textLabel.text=@"业务事项";
            }
            cell1.selectionStyle = UITableViewCellSelectionStyleNone;//不可点击
        }
        if (indexPath.row == 1) {
            [cell1.contentView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
            //菜单功能中的一项
            
           UIWebView *webview=[[UIWebView alloc] initWithFrame:cell1.contentView.frame];
