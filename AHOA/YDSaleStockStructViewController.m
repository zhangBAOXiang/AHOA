//
//  YDSaleStockStructViewController.m
//  AHOA
//
//  Created by 224 on 15/9/14.
//  Copyright (c) 2015年 zoomlgd. All rights reserved.
//

#import "YDSaleStockStructViewController.h"
#import "YDBaseViewController+ConfigureCell.h"
#import "YDOperation.h"
#import "AHKActionSheet.h"
#import "YDSaleStock.h"

@interface YDSaleStockStructViewController ()<UITableViewDataSource,UIScrollViewDelegate,UITableViewDelegate,YDSOAPDelegate>
...
@implementation YDSaleStockStructViewController
...
- (void)viewDidLoad
{
    ...
    [self.operation getProSaleStockWithType:@"99"];
...
- (void)customButton
{
    ...
    [btn1 setTitle:@"总体" forState:UIControlStateNormal];
    //    [btn1 setTitleColor:UIColorFromRGB(0x6e6e6e) forState:UIControlStateNormal];
    [btn1 addTarget:self action:@selector(mainTapped:) forControlEvents:UIControlEventTouchUpInside];
    [btn1 setBackgroundColor:[UIColor clearColor]];
    [btn1 setTag:1001];
//    [btn1 setHighlighted:YES];
    
    YDImageButton *btn3=[YDImageButton buttonWithType:UIButtonTypeCustom];
    [btn3 setFrame:CGRectMake(2*margin+80, 0, 80, 48)];
...
{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSData *source = [defaults objectForKey:@"structstocks"];
    if (source) {
        self.isDoubleClick = YES;
        self.leftSource = self.rightSource = [NSKeyedUnarchiver unarchiveObjectWithData:source];
        [self.leftTable reloadData];
        [self.rightTable reloadData];
        [defaults removeObjectForKey:@"structstocks"];
...
- (void)doubleClick:(UITapGestureRecognizer *)recognizer {
    if (self.isDoubleClick) {
        ...
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:[NSKeyedArchiver archivedDataWithRootObject:self.leftSource] forKey:@"structstocks"];
        [defaults synchronize];
        [self.HUD show:YES];
